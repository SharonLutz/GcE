GcE <-
function(nCase=407,nControl=376,MAF=0.49,meanE=0,varE=0.99,beta0=-0.32,betaSNP=0.17,betaE=0.97,betaI=seq(-1,-0.75,by=0.05),nSim=1000,alpha=0.00000005,plot.output=TRUE,plot.name="GcE.pdf",seed=1){ 
    ...
    colnames(rejectH0)<-c("BetaI","propSim")
    ...
    for(GLOBALVAR in 1:nSim){
      ...
      for(bb in 1:length(betaI)){
        ...
        if(modelI[4,4]<alpha){rejectH0[bb,"propSim"]<-rejectH0[bb,"propSim"]+1}
    ...
    rejectMat<-cbind(rejectH0[,"BetaI"],rejectH0[,"propSim"]/nSim)
    colnames(rejectMat)<-c("BetaI","propSim")
    ...
    if(plot.output==TRUE){
      pdf(plot.name)
      plot(-1,-1,xlim=c(min(betaI),max(betaI)),ylim=c(0,1),xlab=expression(beta[I]),ylab=expression("Proportion of Simulations" ~ H[0] ~ "rejected"),main="")
